...
const ForgotPassword: React.FC = () => {
    ...
    const [lastRequestTime, setLastRequestTime] = useState(() => {
        const saved = localStorage.getItem('passwordResetLastRequest');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (cooldown > 0) {
            setError(`Please wait ${formatTimeRemaining(cooldown)} before requesting another reset`);
            return;
        }
                            ...
                            <div>
                                ...
                                {cooldown > 0 && !success && (
                                    <p className="mt-2 text-amber-400 text-xs">
                                        You can request another reset in {formatTimeRemaining(cooldown)}
                                    </p>
                                )}
                            </div>
